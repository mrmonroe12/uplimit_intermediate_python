...
def batch_files(file_paths: List[str], n_processes: int) -> List[set]:
    if n_processes > len(file_paths):
        return [set(path) for path in file_paths]

    n_per_batch = math.floor(len(file_paths)/n_processes)
    ...
    batches[0] = batches[0].union(set(second_set))
...
def main() -> List[Dict]:
    ...
    print('batches:',len(batches),'n_processes:',n_processes)

    ######################################## YOUR CODE HERE ##################################################
    with multiprocessing.Pool(processes=n_processes) as pool:
        revenue_data = pool.starmap(run,[(file_batch, batch_num) for batch_num, file_batch in enumerate(batches)])
        pool.close()
        pool.join()
